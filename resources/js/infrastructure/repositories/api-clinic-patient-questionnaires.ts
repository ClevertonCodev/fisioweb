import type { PatientQuestionnairesRepository, PatientQuestionnaireWriteDto } from '@/application/clinic/ports';
import type { PatientQuestionnaire, PatientQuestionnaireDetail } from '@/domain/clinic';
...
function toPatientQuestionnaire(raw: ApiPatientQuestionnaireDto): PatientQuestionnaire {
    return {
        id: raw.id,
        ...
        template: raw.template ? { id: raw.template.id, title: raw.template.title } : undefined,
        status: raw.status,
...
export const apiClinicPatientQuestionnairesRepository: PatientQuestionnairesRepository = {
    async listByPatient(patientId) {
        const res = await apiClient.get<{ data: ApiPatientQuestionnaireDto[] }>(
    ...
    async store(patientId, dto: PatientQuestionnaireWriteDto) {
        const res = await apiClient.post<{ data: ApiPatientQuestionnaireDto }>(
            `/clinic/patients/${patientId}/questionnaires`,
            {
    ...
    async destroy(patientId, questionnaireId) {
        await apiClient.delete(`/clinic/patients/${patientId}/questionnaires/${questionnaireId}`);
    },
};
