import type { ClinicUserUpdateDto, ClinicUserWriteDto, ClinicUsersRepository } from '@/application/clinic/ports';
...
function toApiPayload(dto: ClinicUserWriteDto | ClinicUserUpdateDto): Record<string, unknown> {
    ...
    if ('password' in dto && dto.password !== undefined) payload.password = dto.password;
    if ('role' in dto && dto.role !== undefined) payload.role = dto.role;
    if ('document' in dto && dto.document !== undefined) payload.document = dto.document;
    if ('status' in dto && dto.status !== undefined) payload.status = dto.status;
    return payload;
}
...
export const apiClinicUsersRepository: ClinicUsersRepository = {
    ...
    async list(): Promise<ClinicUserSummary[]> {
        const { data } = await apiClient.get<{ data: ApiClinicUser[] }>('/clinic/users');
    ...
    async update(id: string, dto: ClinicUserUpdateDto): Promise<ClinicUserSummary> {
