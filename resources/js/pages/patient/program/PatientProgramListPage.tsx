...
export default function PatientProgramListPage() {
    const { data: programs, isLoading, refetch, isRefetching } = usePatientPrograms();
    ...
    return (
        <div className="flex h-screen flex-col bg-slate-50 pb-20 overflow-y-auto">
            <header className="sticky top-0 z-10 flex items-center justify-between bg-primary px-4 py-4 text-primary-foreground shadow-md">
                ...
                <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-primary/90" onClick={() => refetch()} disabled={isRefetching}>
            ...
            <main className="flex-1 p-4">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
                        ...
                        <p>Nenhum programa disponível.<br />Entre em contato com o profissional da saúde.</p>
                    </div>
                ) : (
                        ...
                        {programs.map((program) => {
                            ...
                            return (
                                <Card 
                                    ...
                                    onClick={() => navigate(`/detalhe-programa?id=${program.publicToken}`)}
                                >
                                    <CardContent className="p-4">
                                        ...
                                        <div className="mb-3 flex items-center text-sm text-slate-600">
                                            ...
                                            <span>{totalExercises} {totalExercises === 1 ? 'exercício' : 'exercícios'}</span>
                                        </div>
                                        
                                        <div className="text-xs text-slate-500">
                                            <p>Profissional: {program.professionalName}</p>
                                            <p>Período: {new Date(program.startDate).toLocaleDateString('pt-BR')} a {new Date(program.endDate).toLocaleDateString('pt-BR')}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            );
